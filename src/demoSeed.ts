...
export const DEMO_PLANNING_RESPONSE: PlanningResponse = {
  summary: 'TODO 항목을 VS Code workspace 안에서 저장하고 다시 불러오는 기능을 추가하는 demo flow입니다. 실제 파일 변경 없이 Planning Gate, Decision Log, Validation, 학습 자료 생성 흐름을 안정적으로 보여줍니다.',
  assumptions: [
  ...
  questions: [
    {
      ...
      reason: '저장 위치는 데이터 지속성, 파일 경로 안전성, 이후 학습 자료 근거에 직접 영향을 줍니다.',
    ...
    },
    {
      ...
      reason: '명령 이름과 동작은 public contract로 남아 이후 사용법과 학습 자료 설명에 영향을 줍니다.',
    ...
    },
    {
      ...
      risk_if_wrong: '검증 실패 또는 미실행 상태가 학습 자료에 충분히 드러나지 않을 수 있습니다.',
      risk_categories: ['code_evidence_lack', 'learning_value'],
...
export const DEMO_TUTORIAL_MARKDOWN = [
  ...
  '- [사용자 결정] 선택 질문은 TODO 데이터를 어디에 저장할지였습니다.',
  '',
  '## 선택 근거',
  '- [AI 추론] 저장 위치는 데이터 지속성과 파일 경로 안전성에 영향을 주기 때문에 REVIEW_REQUIRED로 기록되었습니다.',
  '- [사용자 결정] 선택하지 않은 대안은 VS Code globalState와 외부 DB/API였습니다.',
  '',
  '## 이전 선택과의 연결',
  '- [확인 필요] demo seed에서는 하나의 선택만 학습 자료에 포함하므로 이전 선택과의 반복 패턴은 단정하지 않습니다.',
  '',
  '## 사용자 선택 성향',
  '- [사용자 결정] 선택 라벨에는 workspace, JSON, 파일 기반 저장이 포함됩니다.',
  '- [AI 추론] 이 demo 기록만 기준으로는 장기적 성향을 단정하지 않고, 현재 선택이 로컬 파일 기반 흐름을 택했다는 점만 정리합니다.',
  '',
  '## 관련 결정 로그',
  ...
  '',
  '## 검증 상태',
  '- [검증 결과] typecheck: passed, build: not available, test: not available, lint: not available',
  '',
  '## 다음 작업에서 확인할 점',
